...
exports.getFeedbackAnalysisByBranch = async (req, res) => {
  try {
    ...
    if (feedbacks.length === 0) {
      return res.status(404).json({ message: "No feedback found for the given branch" });
    }
    ...
    const result = Object.keys(facultyAnalysis).map(facultyName => {
      ...
      const averageScore = facultyData.count > 0 ? (facultyData.totalScore / facultyData.count).toFixed(2) : '0.00';
      const averagePercentage = facultyData.count > 0
        ? ((facultyData.totalScore / (facultyData.count * 4)) * 100).toFixed(2) // Convert average score to percentage
    ...
    res.status(500).json({ message: "Error analyzing feedback by branch", error });
  }
};
